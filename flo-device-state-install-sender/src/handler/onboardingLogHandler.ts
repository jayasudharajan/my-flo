...
import { sendToDeviceStateService } from '../deviceStateService/deviceStateServiceSender';
...
export const handleOnboardingLog = async (onboardingLog: OnboardingLog): Promise<void> => {
  ...
  console.log(`Sending 'installed' status to FireWriter Service for Device ID ${onboardingLog.icd_id} / ${deviceInfo.device_id}.`);
  await sendToDeviceStateService(deviceInfo.device_id);
  console.log(`Successfully sent 'installed' status to FireWriter Service for Device ID ${onboardingLog.icd_id} / ${deviceInfo.device_id}.`);
}