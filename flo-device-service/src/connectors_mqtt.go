...
func initMqttClient(clientId string) (mqtt.Client, error) {
	...
	ops := mqtt.NewClientOptions().SetClientID(clientId).AddBroker(MqttBrokerUrl).SetTLSConfig(tlsConfig).SetCleanSession(true)
	...
	log.Infof("MQTT client %s has been initialized, connected to %s broker", clientId, MqttBrokerUrl)
