...
func NewAppContext(log Log, codeHash, codeTime, codeBranch string) *appContext {
	a := appContext{
		log:        log,
		Start:      time.Now(),
		App:        getEnvOrDefault("APPLICATION_NAME", getEnvOrDefault("INSTANA_SERVICE_NAME", "alarm-dotcom")),
...
func (a *appContext) IsProd() bool {
	return strings.Index(strings.ToLower(a.Env), "prod") == 0
}
