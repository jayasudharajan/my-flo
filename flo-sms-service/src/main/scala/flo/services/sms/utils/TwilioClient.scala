    ...
    try {
      ...
      if (!sms.deliveryCallback.isEmpty) {
        val adaptedMessage = TwilioMessage(
          ...
          customFormat.print(Option(message.getDateSent).getOrElse(DateTime.now())),
          Option(message.getErrorCode).map(_.toInt).getOrElse(-1),
          Option(message.getErrorMessage).getOrElse(""),
