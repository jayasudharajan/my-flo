...
// them through the Kafka requests made with kafka producers.
// func ProducerMessageWithSpan(pm *kafka.Message, sp ot.Span) *kafka.Message {
// 	sp.Tracer().Inject(sp.Context(), ot.TextMap, ProducerMessageCarrier{Message: pm})
// 	return pm
// }

func ProducerMessageWithSpan(pm *kafka.Message, sp ot.Span) *kafka.Message {
	pmc := ProducerMessageCarrier{Message: pm}
	err := sp.Tracer().Inject(sp.Context(), ot.TextMap, pmc)
	if err == nil {
		// if there is actually instana trace headers
		isc, ok := sp.Context().(instana.SpanContext)
		if ok {
			// correct instana code in go-sensor@1.55.2/propagation.go:88
			pmc.Set(instana.FieldT, instana.FormatLongID(isc.TraceIDHi, isc.TraceID))
		}
	}

	// add w3c trace contexts too if present
	if isc, ok := sp.Context().(instana.SpanContext); ok {
		if len(isc.W3CContext.RawParent) > 0 {
			pm.Headers = append(pm.Headers, kafka.Header{Key: w3ctrace.TraceParentHeader, Value: []byte(isc.W3CContext.RawParent)})
		}
		if len(isc.W3CContext.RawState) > 0 {
			pm.Headers = append(pm.Headers, kafka.Header{Key: w3ctrace.TraceStateHeader, Value: []byte(isc.W3CContext.RawState)})
		}
	}
...
func ProducerMessageWithSpanFromContext(ctx context.Context, pm *kafka.Message) *kafka.Message {
...
func (c ProducerMessageCarrier) RemoveAll() {
	...
	for _, header := range c.Message.Headers {
		if bytes.EqualFold([]byte(header.Key), fieldCKey) ||
			bytes.EqualFold([]byte(header.Key), fieldLKey) ||
			bytes.EqualFold([]byte(header.Key), fieldTKey) ||
			bytes.EqualFold([]byte(header.Key), fieldSKey) ||
			bytes.EqualFold([]byte(header.Key), fieldLSKey) {
...
func (c ProducerMessageCarrier) ForeachKey(handler func(key, val string) error) error {
	...
	for _, header := range c.Message.Headers {
		...
		case bytes.EqualFold([]byte(header.Key), fieldCKey) && kafkaHeaderFormat != bothFormat:
			traceID, spanID, err := UnpackTraceContextHeader(header.Value)
			if err != nil {
				return fmt.Errorf("malformed %q header: %s", []byte(header.Key), err)
			}
		...
		case bytes.EqualFold([]byte(header.Key), fieldLKey) && kafkaHeaderFormat != bothFormat:
		...
		case bytes.EqualFold([]byte(header.Key), fieldTKey):
			if err := handler(instana.FieldT, string(header.Value)); err != nil {
				return err
			}
		case bytes.EqualFold([]byte(header.Key), fieldSKey):
			if err := handler(instana.FieldS, string(header.Value)); err != nil {
				return err
			}
		case bytes.EqualFold([]byte(header.Key), fieldLSKey):
...
func (c ProducerMessageCarrier) addOrReplaceHeader(key, val []byte) {
	...
	c.Message.Headers = append(c.Message.Headers, kafka.Header{Key: string(key), Value: val})
}

func (c ProducerMessageCarrier) indexOf(key []byte) (int, bool) {
	for i, header := range c.Message.Headers {
		if bytes.EqualFold(key, []byte(header.Key)) {
			return i, true
		}
	}
...
func SpanContextFromConsumerMessage(cm *kafka.Message, sensor instana.TracerLogger) (ot.SpanContext, error) {
	spanContext, err := sensor.Tracer().Extract(ot.TextMap, ConsumerMessageCarrier{cm})
	if err != nil {
		// see if w3c trace context is there
		var w3cctx w3ctrace.Context
		for _, h := range cm.Headers {
			switch h.Key {
			case w3ctrace.TraceParentHeader:
				w3cctx.RawParent = string(h.Value)
			case w3ctrace.TraceStateHeader:
				w3cctx.RawState = string(h.Value)
			}
		}

		if len(w3cctx.RawParent) > 0 {
			p, err := w3ctrace.ParseParent(w3cctx.RawParent)
			if err != nil {
				return nil, err
			}
			traceHi, traceLo, err := instana.ParseLongID(p.TraceID)
			if err != nil {
				return nil, err
			}
			spanId, err := instana.ParseID(p.ParentID)
			if err != nil {
				return nil, err
			}
