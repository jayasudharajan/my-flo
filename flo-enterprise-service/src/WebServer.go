...
func CreateWebServer(validator *Validator, log *Logger, registerRoutes func(*WebServer), closers []Resource) *WebServer {
	if log == nil {
		log = DefaultLogger()
	}
...
func (ws *WebServer) tryClose(c Resource, n int) {
	if c == nil {
		return
	}
	defer panicRecover(ws.log, "tryClose %v", n)
	ws.log.Trace("tryClosing #%v: %v", n, TypeName(c))
	c.Close()
}
...
func (ws *WebServer) initInstana() *WebServer {
	...
	// add middleware after Instana is initialized
