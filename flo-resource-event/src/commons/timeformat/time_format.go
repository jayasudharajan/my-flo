...
func (ct *TimeFormat) UnmarshalJSON(b []byte) (err error) {
	defer utils.PanicRecover(utils.Log(), "TimeFormat.UnmarshalJSON: %s", b)
...
func (ct TimeFormat) MarshalJSON() ([]byte, error) {
...
func (ct TimeFormat) String() string {
	return ct.Format(TIME_FMT_NO_TZ + ".000000")
}

func (ct *TimeFormat) Format(fmtStr string) string {
	if ct == nil {
...
func (ct *TimeFormat) Time() time.Time {
	if ct == nil {
		return time.Time{}
	}
	return time.Time(*ct)
...
func (ct *TimeFormat) UTC() time.Time {
	return ct.Time().UTC()
}
