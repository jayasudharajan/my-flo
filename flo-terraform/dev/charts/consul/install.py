...
tpl_s = os.system("helm template " + FLO_HELM_REPO_NAME + "/consul --namespace consul --values values-dev.yaml | tee output.yaml")
if tpl_s != 0:
    exit(tpl_s)

os.system("helm upgrade --install consul " + FLO_HELM_REPO_NAME + "/consul --wait --namespace consul --values values-dev.yaml")
