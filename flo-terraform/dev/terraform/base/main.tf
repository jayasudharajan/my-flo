module "kops" {
  ...
  route53_domain = "k8s.flocloud.${var.global["domain"]}"
  kops_bucket_name = "${var.global["organisation"]}-${var.global["project"]}-kops"
  ...
  tag_kubernetes_cluster = "k8s.flocloud.co"
...
locals {
  cluster_name = "k8s.flocloud.${var.global["domain"]}"
}
