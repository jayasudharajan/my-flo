...
var _log *Logger = NewLogger(getEnvOrDefault(envVarAppName, defaultLogPrefix),
	getEnvOrDefault(envVarLogMinLevel, ""),
	environmentConst(),
	strings.ToLower(getEnvOrDefault(envVarLocalDebug, "")) == "true")

func NewLogger(logPrefix, minLogLevel, environment string, localDebug bool) *Logger {
	min := minLogLevel
