...
func CreateWebServer(log *Logger, registerRoutes func(*WebServer), closers []ICloser) *WebServer {
	if log == nil {
		log = DefaultLogger()
	}
	...
	if ws.validate, e = NewValidator(_loggerSbPool); e != nil {
		log.IfErrorF(e, "CreateWebServer: can't create validator")
		return nil
	}
	ws.presetLogIgnores()
...
func (ws *WebServer) HttpError(c *gin.Context, code int, msg string, err error) error {
	...
	if err != nil {
		if he, ok := err.(*HttpErr); ok {
			rv = he
			rv.Message = fmt.Sprintf("%s. %s", msg, rv.Message)
		} else {
			rv.Trace = err
		}
	}
...
func (ws *WebServer) HttpReadBody(c *gin.Context, v interface{}) error {
	...
	if isNil := r.Body == nil; isNil || r.ContentLength <= 0 {
		return &HttpErr{400, "Empty body", nil}
	} else if !isNil {
		defer r.Body.Close()
	}
	...
	if err != nil {
		return &HttpErr{400, "Can't parse body", err}
	}
...
func (ws *WebServer) HttpReadQuery(c *gin.Context, v interface{}) error {
	...
	if err := decoder.Decode(v, c.Request.URL.Query()); err != nil {
		return &HttpErr{400, "Can't parse query", err}
	}
	if err := ws.validate.Struct(v); err != nil {
		return err
	}
