...
type Logger struct {
	...
	childLevel int
	isDebug    bool
	sbPool     *sync.Pool //recycling of sb to lower heap trashing & reduce gc work
}

const envVarLocalDebug = "FLO_LOCAL_DEBUG"
const envVarLogMinLevel = "FLO_LOG_MIN_LEVEL"
const envVarAppName = "APPLICATION_NAME"
const logPrefixDefault = "flo-task-scheduler"

func DefaultLogger() *Logger {
	min := getEnvOrDefault(envVarLogMinLevel, "")
	n, _ := strconv.Atoi(min)
	if n > int(LL_ERROR) && n < 0 {
		n = 0
	}
	isDebug := strings.ToLower(getEnvOrDefault(envVarLocalDebug, "")) == "true"
	isLocal := getEnvOrDefault("ENV", getEnvOrDefault("ENVIRONMENT", "local")) == "local"
	prefix := getEnvOrDefault(envVarAppName, logPrefixDefault)
	log := CreateLogger(prefix, "", LogLevel(n)).SetColor(isDebug || isLocal)
	log.isDebug = isDebug
	return log
}

var _loggerSbPool *sync.Pool
var _loggerLock = sync.Mutex{}

func initLoggerPool() *sync.Pool {
	if _loggerSbPool == nil { //double check lock, lazy singleton
		_loggerLock.Lock()
		defer _loggerLock.Unlock()
		if _loggerSbPool == nil {
			_loggerSbPool = &sync.Pool{
				New: func() interface{} {
	...
	return _loggerSbPool
}

func CreateLogger(prefix, name string, minLevel LogLevel) *Logger {
	if prefix == "" {
...
const logWrapTemplate = " | %v"

func (l *Logger) IfFatalF(e error, format string, args ...interface{}) error {
	if e != nil {
		return l.Fatal(format+logWrapTemplate, append(args, e.Error())...)
	}
	return e
}
...
func (l *Logger) IfErrorF(e error, format string, args ...interface{}) error {
	if e != nil {
		return l.Error(format+logWrapTemplate, append(args, e.Error())...)
	}
	return e
}
...
func (l *Logger) IfWarnF(e error, format string, args ...interface{}) error {
	if e != nil {
		return l.Warn(format+logWrapTemplate, append(args, e.Error())...)
	}
	return e
}
...
func wrapErrors(es []error) error {
	...
	pool := initLoggerPool() //borrow logger sb pool since we have it here
	var sb *strings.Builder
	if o := pool.Get(); o != nil { //pull 1 from the pool
		sb = o.(*strings.Builder)
		sb.Reset()
	} else { //just make one
		sb = &strings.Builder{}
