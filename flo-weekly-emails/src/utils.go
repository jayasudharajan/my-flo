...
func tryToJson(item interface{}) string {
	defer panicRecover(_log, "tryToJson: %v", TypeName(item))
	if item == nil {
		return ""
	}
...
func coalesceFloat32(p *float32) float32 {
	if p == nil {
		return 0
	}
	return *p
...
func coalesceFloat64(p *float32) float64 {
	if p == nil {
		return 0
	}
	return float64(*p)
...
func pointToFloat32(val float32) *float32 {
	return &val
}
