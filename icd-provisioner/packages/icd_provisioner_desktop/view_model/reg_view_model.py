...
class RegViewModel(StoppableViewModel):
    ...
    def on_device_id_changed_listener(self, device_id: str):
        ...
        self.startable.set_value(sdk.is_valid_device_id(device_id) 
    ...
    def on_mfg_sn_changed_listener(self, mfg_sn: str):
        self.clear_progresses()
        mfg_sn_count: int = 0 if config.get_config().mfg_sn_count is None else config.get_config().mfg_sn_count
        self.startable.set_value(sdk.is_valid_device_id(self.device_id.get_value()) 
    ...
    def on_model_done_listener(self, reg_task_enum: enum.Enum, result: str, error: str):
        if reg_task_enum in self.progress:
            self.progress[reg_task_enum].set_value(error if error else result)

        if error or sdk.is_final_enum(reg_task_enum) \
                or (not self.parent.reg_print.get_value() and reg_task_enum == model.RegTask.LOCK):
            ...
            report.report(report.Report(
                ...
                operator_id=self.parent.reg_operator_id.get_value(),
                content=TasksStatus.FAILED.name if error else TasksStatus.PASSED.name,
                timestamp=datetime.datetime.now()))

    def on_model_progress_listener(self, reg_task_enum: enum.Enum, progress: str):
        if reg_task_enum in self.progress:
            self.progress[reg_task_enum].set_value(progress)

    def start_process(self):
        self.tasks_status.set_value(TasksStatus.PROCESSING.value)
        reg_model: model = model.TaskChainModel(
            model.get_reg_model_qrunnables(
                self.device_id.get_value(),
                self.parent.reg_print.get_value(),
                self.parent.reg_wait_for_printing.get_value(),
