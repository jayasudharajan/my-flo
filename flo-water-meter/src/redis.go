...
func parseRedisCn(cn string) (*redis.UniversalOptions, error) {
	...
	if addrLen > 1 { //cluster mode
		//cfg.RouteRandomly = true //randomly distribute read amongst primary & secondary to reduce load on primary
...
func (rc *RedisConnection) Ping() error {
	if rc == nil {
		return logFatal("Redis Ping -> Not init: Nil ref!")
	}
	if pong, err := rc._client.Ping().Result(); err != nil {
		return logError("Redis Ping -> %v", err)
	} else if len(pong) == 0 {
		return logError("Redis Ping -> nil")
	} else {
		return nil
	}
}
