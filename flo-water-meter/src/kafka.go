...
func (k *KafkaConnection) Ping() error {
	if k == nil {
		return logFatal("Kafka Ping -> Not init: Nil ref!")
	}
	if e := k.Producer.GetFatalError(); e != nil {
		return logError("Kafka Ping -> %v", e)
	} else {
		return nil
	}
}
...
func (k *KafkaConnection) Subscribe(groupId string, topics []string, processFunction KafkaMessageConsumer) (*KafkaSubscription, error) {
	...
	go func(cn *KafkaSubscription) {
		...
		for {
			...
			// Cancel signalled
			if atomic.LoadInt32(&cn.cancel) > 0 {
				return
			}
		}
	}(rv)
...
func (ks *KafkaSubscription) Close() {
	if ks == nil {
		return
	}

	atomic.AddInt32(&ks.cancel, 1)
	if ks.parent != nil {
