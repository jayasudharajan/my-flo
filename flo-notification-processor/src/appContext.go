...
func NewAppContext(log *Logger, codeHash, codeTime, codeBranch string) *appContext {
	a := appContext{
		log:        log.Clone().SetName("appCtx"),
		Start:      time.Now(),
		App:        getEnvOrDefault("APPLICATION_NAME", getEnvOrDefault("INSTANA_SERVICE_NAME", "notification-processor")),
