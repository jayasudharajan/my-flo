...
func (ws *apiServer) getServiceName() string {
...
func NewWebServer(validator Validator, log *Logger, registerRoutes func(WebServer, gin.IRoutes), closers []OpenCloser) WebServer {
	if log == nil {
		log = DefaultLogger()
	}
	...
	ws.presetLogIgnores()
	sn := ws.getServiceName()
	ws.instana = _instana
	ws.tp = _tracerProvider
	ws.router.Use(otelgin.Middleware(sn)) // for otel signals
...
func (ws *apiServer) Close() {
	...
	wg := sync.WaitGroup{}
	for i, c := range ws.closers {
		wg.Add(1) // put this outside goroutine to avoid race conditions
	...
	if err := ws.tp.Shutdown(context.Background()); err != nil {
		ws.log.IfWarn(err)
	}
	wg.Wait()
}

func (ws *apiServer) Log() *Logger {
	if ws == nil {
		return nil
	}
	return ws.log
