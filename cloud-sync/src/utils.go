...
func RetryIfErrorLimitAttempts(ctx context.Context, f func(ctx context.Context) error, interval time.Duration, attemptsLeft int, log *Logger) error {
	if f == nil {
		return nil
	}
	...
	time.Sleep(interval)
	return RetryIfErrorLimitAttempts(ctx, f, interval, updatedAttemptsLeft, log)
}
