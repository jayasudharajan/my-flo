...
// WebServer wrapper for gin framework for common web hosting tasks
...
func (ws *WebServer) Open(ctx context.Context) *WebServer {
	if ws == nil {
		return nil
	}
...
func (ws *WebServer) Close(ctx context.Context) {
	...
	wait := time.Second * 3
	tctx, cancel := context.WithTimeout(context.Background(), wait)
	defer cancel()

	if err := ws.httpSvr.Shutdown(tctx); err != nil {
		ws.log.IfWarn(err)
	}
	for i, c := range ws.closers {
		ws.tryClose(ctx, c, i)
...
func (ws *WebServer) tryClose(ctx context.Context, c ICloser, n int) {
	if c == nil {
		return
	}
...
func (ws *WebServer) initInstana(sl ServiceLocator) *WebServer {
	...
	// add middleware after Instana and services are initialized
