...
func (event *VEvent) SetRepeatDailyRule(days []string, props ...PropertyParameter) {
	if len(days) == 0 {
		return
	}
	if len(days) == 7 {
		event.SetProperty(ComponentRepeatRule, fmt.Sprintf("FREQ=DAILY;INTERVAL=1"), props...)
	} else {

		event.SetProperty(ComponentRepeatRule,
			fmt.Sprintf("FREQ=DAILY;BYDAY=%v", strings.ToUpper(strings.Join(days, ","))),
			props...)
	}
}
